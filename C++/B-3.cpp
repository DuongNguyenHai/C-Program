		...
		if( 'a'<=s[i] && s[i]<='z' || 'A'<=s[i] && s[i]<='Z' || '0'<=s[i] && s[i]<='9' ){
			...
			while(s[j]!='\0'){
				if(i!=j) // bo qua truong hop so sanh voi chinh minh
					if( s[i]==s[j] || s[i]==s[j]-('a'-'A') || s[i]==s[j]+('a'-'A') )	// giong nhau, theo bang ascii, 'a' cach 'A' 1 khoang = 'a'-'A'
						solan++;
				j++;
			}
			// printf("solan: %d\n",solan);
			...
			for(j=0;j<tskt;j++){
				if( s[i]==kt[j] || s[i]==kt[j]-('a'-'A') || s[i]==kt[j]+('a'-'A') )
					break;
			}
...
void doctep(char s[]){
	FILE *pfile;
	int i=0;
	pfile = fopen("file/B-3-read.txt","r");
...
void ghitep(char kt[],int sl[], int tskt){
	FILE *pfile;
	pfile = fopen("file/B-3-write.txt","w");
	if(pfile==NULL){
		perror ("Error opening file");
		exit(-1);
	}
...
int main(){
	...
	printf("Khong phan biet chua hoa chu thuong\n");
	printf("kt : sl\n");
	for(int i=0;i<tskt;i++){
		printf("%c  :  %d\n",kt[i],sl[i]);
	}
